...
- **Billing docs corrected for the 2026-06-15 subscription change.** Headless
  ...
  **Agent SDK Credit** (Pro $20 / Max $100–200), then pay-as-you-go — it is *not*
  deprecated. Updated the runbook intro + Track C and the `fa.env` /
  `property-agent.env` agent-provider comments, which now also warn: do **not**
  set `ANTHROPIC_API_KEY` in a `claude_cli` edition — it silently bills API
