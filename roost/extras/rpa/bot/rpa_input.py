...
async def handle_rpa_choice_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Resume an awaiting_input run from an inline-keyboard button tap.

    Triggered by `callback_data` of the form `rpa_choice:<run_id>:<value>` sent by
    the `telegram_send` step op when a `keyboard` is supplied.
    """
    query = update.callback_query
    if not query or not query.data:
        return
    parts = query.data.split(":", 2)
    if len(parts) != 3 or parts[0] != "rpa_choice":
        return
    try:
        run_id = int(parts[1])
    except ValueError:
        await query.answer("Invalid run id", show_alert=False)
        return
    value = parts[2]

    # Scope: only the user who owns the run may answer for it. Mirrors the
    # text-input handler's user-resolution pattern so a fellow allowed user
    # can't drive another's RPA flow by tapping their keyboard.
    run = rpa_runs.get_run(run_id)
    if not run:
        await query.answer("Run not found", show_alert=True)
        return
    tapping_user = _resolve_user_id(update.effective_user.id)
    if str(run.get("user_id")) != tapping_user:
        await query.answer("Not your run", show_alert=True)
        return

    if rpa_runs.submit_input(run_id, value):
        await query.answer(f"Got it: {value[:30]}")
        try:
            await query.edit_message_reply_markup(reply_markup=None)
        except Exception:
            pass
    else:
        await query.answer("Run is no longer awaiting input", show_alert=True)
