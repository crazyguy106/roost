...
logger = logging.getLogger("roost.extras.rpa.services.rpa_runs")
...
def mark_awaiting_input(run_id: int, prompt_text: str, kind: str) -> None:
    """Mark the run as awaiting input and arm the asyncio event.

    Public for step ops that want to send their own custom prompt (e.g. a
    Telegram inline-keyboard message) BEFORE awaiting — the pause flag must
    be set first so a fast user response isn't dropped by `submit_input`.
    Pair with `await_input_event`.
    """
    conn = get_connection()
    try:
        conn.execute(
...
async def await_input_event(run_id: int, kind: str, timeout: float = 300.0) -> str:
    """Block until `submit_input` (or cancel) fires the run's event.

    Must be preceded by `mark_awaiting_input` to arm the event. Returns the
    value the user supplied. Raises `asyncio.TimeoutError` on timeout (also
    marks the run failed), or `RuntimeError` if the run is cancelled mid-wait.
    """
    event = _events.get(run_id)
    if not event:
        raise RuntimeError(f"Run {run_id} not armed for input — call mark_awaiting_input first")
...
async def request_input(
    ...
    """Pause the run, send a text-only Telegram nudge, return the user's answer.

    For richer prompts (inline keyboards, attached screenshots) call
    `mark_awaiting_input` + send your own message + `await_input_event` so
    the pause is in place before the user can react.

    Raises:
      asyncio.TimeoutError if the user does not respond in `timeout` seconds.
      RuntimeError if the run is cancelled while awaiting.
    """
    mark_awaiting_input(run_id, prompt_text, kind)
    await _notify_telegram(run_id, prompt_text, kind)
    return await await_input_event(run_id, kind, timeout)
