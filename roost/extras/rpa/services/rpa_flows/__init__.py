...
from roost.extras.rpa.services.rpa_flows.loader import (
    LIBRARY_DIR,
    USER_DIR,
    dump_yaml,
    export_from_db,
...
from roost.extras.rpa.services.rpa_flows.schema import FlowValidationError, validate_flow
...
def __getattr__(name):
    ...
    if name in ("run_config", "run_single_step"):
        from roost.extras.rpa.services.rpa_flows._interpreter import run_config, run_single_step
        return {"run_config": run_config, "run_single_step": run_single_step}[name]
    raise AttributeError(f"module 'roost.extras.rpa.services.rpa_flows' has no attribute {name!r}")
...
async def dispatch(portal: str, run_id: int, user_id: str, params: dict) -> dict:
    ...
    # Top-level YAML fields like `login_url` are addressable from steps as
    # `$param:login_url`. Seed them into params here so step authors don't
    # have to re-pass them at every rpa_run() call. Caller-supplied params win.
    params = dict(params or {})
    for key in ("login_url",):
        val = cfg.get(key)
        if val and not params.get(key):
            params[key] = val
    from roost.extras.rpa.services.rpa_flows._interpreter import run_config as _run_config
