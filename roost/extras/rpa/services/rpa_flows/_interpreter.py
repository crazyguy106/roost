...
from roost.services import archive_service
from roost.extras.rpa.services import browser_service, rpa_runs
from roost.services.credentials import get_credential
from roost.extras.rpa.services.otp_source import EmailOtpSource, TelegramOtpSource
...
async def _step_await_user_session(page, step, ctx):
    ...
    live_url = None
    try:
        from roost.extras.rpa.services.browser_service import live_debug_url
    ...
    try:
        from roost.extras.rpa.services.rpa_runs import set_state_field
    ...
    try:
        from roost.extras.rpa.services.rpa_runs import _notify_telegram
...
async def _step_whatsapp_send(page, step, ctx):
    ...
    """
    from roost.extras.messaging_external.services import whatsapp as wa
...
async def _step_telegram_send(page, step, ctx):
    """Send a Telegram message via the Bot HTTP API.

    Mirrors `whatsapp_send` for media resolution. Defaults `to` to the first
    `TELEGRAM_ALLOWED_USERS` entry when omitted. Photo branch is JPEG-compressed
    by Telegram (fine for previews) — pass `as: document` to ship the file
    byte-for-byte (required for scannable QR codes / barcodes).

    When `keyboard` is set, the run is paused until the user taps a button.
    The pause flag is set BEFORE the message ships so a fast tap isn't dropped.
    The selected value is stored under `ctx.vars[<as_var>]` (default
    `last_telegram_choice`).

    Args:
        to: chat_id (defaults to first TELEGRAM_ALLOWED_USERS).
        body: text-only message body.
        caption: caption when sending media.
        image / document: explicit media path.
        source: var name to pull media path from (e.g. "last_screenshot").
        as: "photo" | "document" — override the auto-detected media kind.
        keyboard: list of rows, each row a list of {text, value} dicts. Each
            button's callback_data is `rpa_choice:<run_id>:<value>`.
        as_var: when keyboard is set, store the tapped value under this var.
        timeout: when keyboard is set, max seconds to wait. Default 300.

    Failure semantics:
        Send errors are logged + recorded in ctx.vars.last_telegram_error and
        do NOT abort the flow (matches whatsapp_send). Keyboard timeouts DO
        abort (matches get_otp).
    """
    import httpx
    ...
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_ALLOWED_USERS:
        ctx["vars"]["last_telegram_error"] = "Telegram not configured"
        logger.warning("[run #%d] telegram_send skipped: TELEGRAM_BOT_TOKEN/ALLOWED_USERS unset", ctx["run_id"])
        return

    to_raw = step.get("to")
    chat_id = _resolve_value(to_raw, ctx) if to_raw else str(TELEGRAM_ALLOWED_USERS[0])
    caption_raw = step.get("caption")
    caption = _resolve_value(caption_raw, ctx) if caption_raw else None
    body_raw = step.get("body")
    body = _resolve_value(body_raw, ctx) if body_raw else None
    ...
    explicit_as = step.get("as")
    if explicit_as in {"photo", "image"} and media_path:
        media_kind = "image"
    elif explicit_as == "document" and media_path:
        media_kind = "document"

    keyboard_spec = step.get("keyboard")
    ...
    if keyboard_spec:
        ...
        for row in keyboard_spec:
            buttons: list[dict] = []
            for btn in row:
                btn_text = _resolve_value(btn["text"], ctx)
                btn_value = str(_resolve_value(btn["value"], ctx))
                buttons.append({
                    "text": btn_text,
                    "callback_data": f"rpa_choice:{ctx['run_id']}:{btn_value}",
                })
            rows.append(buttons)
        reply_markup = {"inline_keyboard": rows}

    # Arm the pause BEFORE sending so a fast button tap can't outrace the state write.
    timeout_s = float(step.get("timeout", 300))
    if reply_markup:
        prompt_summary = caption or body or "Choose:"
        rpa_runs.mark_awaiting_input(ctx["run_id"], prompt_summary, "telegram_choice")

    api = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}"
    text_to_send = caption or body or ""

    try:
        async with httpx.AsyncClient(timeout=30) as client:
            if media_kind in {"image", "document"} and media_path:
                endpoint = "sendDocument" if media_kind == "document" else "sendPhoto"
                field = "document" if media_kind == "document" else "photo"
                with open(media_path, "rb") as fh:
                    files = {field: (Path(media_path).name, fh.read(),
                                     "application/octet-stream" if media_kind == "document" else "image/png")}
                data: dict[str, str] = {"chat_id": str(chat_id)}
                if text_to_send:
                    data["caption"] = text_to_send
                if reply_markup:
                    data["reply_markup"] = json.dumps(reply_markup)
                resp = await client.post(f"{api}/{endpoint}", data=data, files=files)
            else:
                if not text_to_send:
                    raise StepError("telegram_send: need one of body / caption / image / document / source")
                payload: dict[str, Any] = {"chat_id": str(chat_id), "text": text_to_send}
                if reply_markup:
            ...
            if msg_id:
                ctx["vars"]["last_telegram_message_id"] = msg_id
    except StepError:
        raise
    except Exception as e:
        ctx["vars"]["last_telegram_error"] = str(e)
        logger.warning("[run #%d] telegram_send failed: %s", ctx["run_id"], e)
        if reply_markup:
            # Send failed but we paused — un-pause so the run can fail cleanly instead of timing out.
            rpa_runs._events.pop(ctx["run_id"], None)
            rpa_runs.fail(ctx["run_id"], f"telegram_send failed before awaiting choice: {e}")
            raise StepError(f"telegram_send: keyboard send failed ({e})")
        return

    if reply_markup:
        try:
            value = await rpa_runs.await_input_event(ctx["run_id"], "telegram_choice", timeout_s)
        except asyncio.TimeoutError:
            ctx["vars"]["last_telegram_error"] = "timeout waiting for button tap"
            raise StepError("telegram_send: timeout waiting for keyboard choice")
        as_var = step.get("as_var", "last_telegram_choice")
        ctx["vars"][as_var] = value
...
HANDLERS = {
    ...
    "telegram_send": _step_telegram_send,
    "log": _step_log,
}

# Fail fast if schema's declared op list ever drifts from the actual handlers.
from roost.extras.rpa.services.rpa_flows.schema import KNOWN_OPS as _SCHEMA_KNOWN_OPS
