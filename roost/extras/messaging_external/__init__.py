...
+ AI CDR (conversational data routing) pipeline.

Mounts:
- Up to 2 API routers: api_whatsapp (gated by WHATSAPP_ENABLED),
  api_wechat (gated by WECHAT_ENABLED)
...
the channel sub-flags (`WHATSAPP_ENABLED`, `WECHAT_ENABLED`) decide
which adapters actually mount.
"""
...
def _register(app: "FastAPI", mcp) -> None:  # noqa: ARG001
    from roost.config import WECHAT_ENABLED, WHATSAPP_ENABLED
