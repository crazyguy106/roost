...
logger = logging.getLogger("roost.extras.crm.services.attio")
...
class AttioProvider(CrmProvider):
    ...
    @staticmethod
    def _name_value(full: str) -> list[dict]:
        """Attio's `name` attribute requires first_name/last_name/full_name.
        Splits on whitespace; everything after the first token is last_name."""
        parts = (full or "").strip().split(None, 1)
        first = parts[0] if parts else ""
        last = parts[1] if len(parts) > 1 else ""
        return [{"first_name": first, "last_name": last, "full_name": full}]

    # Common dial-code → ISO 3166-1 alpha-2 lookup. Attio's phone validator
    # rejects E.164 strings without an explicit country_code, so we derive it
    # from the leading "+CC". Order matters — longest prefixes first.
    _DIAL_PREFIXES = [
        ("+852", "HK"), ("+853", "MO"), ("+886", "TW"),
        ("+1", "US"), ("+44", "GB"), ("+61", "AU"), ("+62", "ID"),
        ("+64", "NZ"), ("+65", "SG"), ("+60", "MY"), ("+66", "TH"),
        ("+84", "VN"), ("+81", "JP"), ("+82", "KR"), ("+86", "CN"),
        ("+91", "IN"), ("+971", "AE"), ("+49", "DE"), ("+33", "FR"),
    ]

    @classmethod
    def _phone_value(cls, raw: str) -> dict:
        """Build an Attio phone-numbers entry. If `raw` starts with a known
        +CC prefix we attach `country_code` (ISO alpha-2) so Attio's
        libphonenumber validator can parse the rest."""
        p = (raw or "").strip()
        for prefix, iso in cls._DIAL_PREFIXES:
            if p.startswith(prefix):
                return {"original_phone_number": p, "country_code": iso}
        return {"original_phone_number": p}

    def create_person(self, *, name=None, emails=None, phones=None, organization_id=None, **extra):
        values: dict[str, Any] = dict(extra)
        if name:
            values["name"] = self._name_value(name)
        if emails:
            values["email_addresses"] = [{"email_address": e} for e in emails]
        if phones:
            values["phone_numbers"] = [self._phone_value(p) for p in phones]
        if organization_id:
    ...
    def update_person(self, person_id: str, **fields):
        ...
        if "name" in fields and fields["name"]:
            values["name"] = self._name_value(fields["name"])
