...
- **FA-edition laptop install** is the Chatwoot-fronted shape: `scripts/install-fa.sh` + `docker-compose.fa.yml` + `env-templates/fa.env` + `tailscale/serve.json`. Runbook at `docs/fa-laptop-install.md`; architecture overview at `docs/fa-edition.md`. The VPS analogue is `docker-compose.fa-vps.yml` (Caddy instead of Tailscale Funnel). Don't add new Chatwoot-bootstrap plumbing; extend what's there.
- **FA-edition WhatsApp outbound is all-Chatwoot.** When `CHATWOOT_ENABLED=true`, `whatsapp.send_text_message` / `send_template_message` / `send_document` / `send_image` all delegate to `chatwoot.route_*_to_whatsapp` — one dispatch path, no Meta-direct surface. Templates use the `template_params` payload, media uses multipart `attachments[]`, both on `POST /conversations/:cid/messages`. `mark_as_read` is a no-op (Chatwoot owns receipts). `link=`/`media_id=` media still error (Chatwoot takes bytes, not URLs). Template discovery via `chatwoot.list_templates(inbox_id)`.
...
| **Messaging external** *(bundle)* — WhatsApp / WeChat / SMS / Chatwoot / AI CDR | `extras/messaging_external/services/{whatsapp,wechat,sms,chatwoot,ai_cdr}.py` | `extras/messaging_external/mcp/tools_whatsapp.py` | `extras/messaging_external/web/{api_whatsapp,api_wechat,api_chatwoot}.py` | `docs/whatsapp-adapter.md`, `docs/sms-adapter.md`, `docs/chatwoot.md` |
| Messaging adapters (Discord/Slack/Signal/Matrix) — standalone processes | `adapters/{discord,slack,signal,matrix}_bot.py` (shared base in `adapters/__init__.py`) | — | — | `docs/messaging-adapters.md` |
...
| **CRM** *(bundle)* — Attio/Zoho/Pipedrive/HubSpot/Salesforce/local | `extras/crm/services/*` | `extras/crm/mcp/tools_crm.py` | `extras/crm/web/{api_crm,api_attio_webhook,auth_zoho}.py` | `docs/crm-adapters.md` |
